...
describe('Frontend Data Contracts & Security Validation (security.md)', () => {
    ...
    it('should validate new Event creation payload structure and preserve attendee schema (EventKit / Google Calendar)', () => {
        // Simulated payload generated by EventFormDialog on new event submission
        const newEventPayload = {
            id: `evt-${Date.now()}`,
            title: "Sprint Review Q3",
            description: "Compromisso sincronizado via Cortex.",
            startTime: "10:00",
            endTime: "10:45",
            duration: "45 min",
            category: "meeting",
        ...
        expect(typeof newEventPayload.id).toBe('string');
        expect(newEventPayload.title.length).toBeGreaterThan(0);
        expect(isSafeMeetingUrl(newEventPayload.meetingLink)).toBe(true);
        expect(['#10B981', '#38BDF8', '#F59E0B', '#A855F7']).toContain(newEventPayload.categoryColor);
        expect(['apple', 'google']).toContain(newEventPayload.destination);
        expect(Array.isArray(newEventPayload.attendees)).toBe(true);
        expect(newEventPayload.attendees.length).toBeGreaterThanOrEqual(1);
        expect(newEventPayload.attendees[0].isYou).toBe(true);
        expect(newEventPayload.attendees[0].status).toBe('accepted');
    });

    it('should strictly preserve existing attendees when editing an event', () => {
        const existingEvent = MOCK_CALENDAR_EVENTS[0];
        const originalAttendeesCount = existingEvent.attendees.length;

        // Simulated edit payload preserving original attendees
        const editedPayload = {
            ...existingEvent,
            title: "Updated Product Architecture Review",
            startTime: "09:00",
            endTime: "10:00",
            duration: "1h",
        ...
        expect(editedPayload.id).toBe(existingEvent.id);
        expect(editedPayload.attendees.length).toBe(originalAttendeesCount);
        expect(editedPayload.attendees).toEqual(existingEvent.attendees);
        expect(editedPayload.organizer).toBe(existingEvent.organizer);
    });

    it('should correctly remove event on deletion and handle selection state clearing', () => {
        let eventsState = [...MOCK_CALENDAR_EVENTS];
        const eventToDelete = eventsState[0];
        let selectedId = eventToDelete.id;

        // Simulated deletion in CalendarTimeline
        eventsState = eventsState.filter(e => e.id !== eventToDelete.id);
        if (selectedId === eventToDelete.id) {
            selectedId = null;
        }

        expect(eventsState.some(e => e.id === eventToDelete.id)).toBe(false);
        expect(eventsState.length).toBe(MOCK_CALENDAR_EVENTS.length - 1);
